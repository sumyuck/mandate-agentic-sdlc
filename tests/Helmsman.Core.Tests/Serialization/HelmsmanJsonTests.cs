...
public sealed class HelmsmanJsonTests
{
    ...
    public void Canonical_serialization_is_byte_stable_across_calls()
    {
        ...
        string first = JsonSerializer.Serialize(sample, HelmsmanJson.Canonical);
        string second = JsonSerializer.Serialize(sample, HelmsmanJson.Canonical);
    ...
    {
        HelmsmanJson.Canonical.IsReadOnly.ShouldBeTrue();
        HelmsmanJson.Pretty.IsReadOnly.ShouldBeTrue();
    }
}
