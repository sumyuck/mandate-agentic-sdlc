...
public sealed class DependencyRuleTests
{
    ...
    public void Orchestrator_references_only_the_core_domain()
    {
        XDocument project = XDocument.Parse(
            RepositoryLayout.ReadProject("src/Helmsman.Orchestrator/Helmsman.Orchestrator.csproj"));
        ...
        references.ShouldBe(["Helmsman.Core"]);
    }

    /// <summary>ADR-0003: only the CLI composition root may bind adapters to ports.</summary>
    [Theory]
    [InlineData("src/Helmsman.Policy/Helmsman.Policy.csproj")]
    [InlineData("src/Helmsman.Persistence/Helmsman.Persistence.csproj")]
    [InlineData("src/Helmsman.Observability/Helmsman.Observability.csproj")]
    [InlineData("src/Helmsman.Llm/Helmsman.Llm.csproj")]
    [InlineData("src/Helmsman.Agents/Helmsman.Agents.csproj")]
    public void Adapters_do_not_depend_on_the_orchestration_engine(string adapterProject)
    {
        ...
        references.ShouldNotContain(reference => reference.Contains("Helmsman.Orchestrator", StringComparison.Ordinal));
    }
